...
const OrderPage = () => {
  ...
  const generatePDF = (order: Order) => {
    ...
    doc.autoTable({
      startY: yPos,
      head: [headers],
      body: [...data, ['', '', '', 'Grand Total', `Rp ${getTotalAmount(order).toLocaleString('id-ID')}`]],
  ...
  {currentItems.map((order, index) => (
    <tr key={order.id} className="border-t border-gray-700 hover:bg-gray-700">
      ...
      <td className="py-3 px-4 flex gap-2">
        ...
        {/* Order Detail Modal with PDF Generation */}
                ...
                <table className="w-full bg-gray-700 rounded-lg">
                  ...
                  <tbody>
                    ...
                    <tr className="font-bold">
                      <td colSpan={4} className="py-2 px-4 text-right">Grand Total:</td>
