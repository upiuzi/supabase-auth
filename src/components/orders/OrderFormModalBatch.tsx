...
interface OrderFormModalProps {
  ...
  batchLabel: string; // tambahkan prop batchLabel
}
...
const OrderFormModal: React.FC<OrderFormModalProps> = ({
  ...
  onAddOrderItem,
  onRemoveOrderItem,
  batchLabel, // tambahkan prop batchLabel
}) => {
  ...
  return (
    <div className="fixed inset-0 bg-gray-900 bg-opacity-75 flex items-center justify-center z-50">
      <div className="bg-gray-800 p-6 rounded-lg w-full max-w-lg text-white shadow-lg mx-2 sm:mx-auto sm:w-full sm:max-w-lg overflow-y-auto max-h-[90vh]">
        <div className="flex justify-between items-center mb-4">
          <div className="flex items-center gap-2">
            <h2 className="text-xl font-bold">{isEdit ? 'Edit Order' : 'Create New Order'}</h2>
            {batchLabel && (
              <span className="ml-2 px-2 py-1 rounded bg-gray-700 text-gray-300 text-base font-medium">
                {batchLabel}
              </span>
            )}
          </div>
        ...
        <form onSubmit={onSubmit}>
          ...
          <div className="grid grid-cols-2 gap-4 mb-4">
            <div>
              ...
              >
                ...
                {companies.map((company) => (
                  <option key={company.id} value={company.id} className="bg-gray-700 text-gray-200">
            ...
            <div>
              <label className="block text-sm font-medium mb-1 text-gray-300">Bank Account</label>
              <select
              ...
              >
                ...
                {bankAccounts
                  .filter((ba) => ba.company_id === formData.company_id)
                  .map((bankAccount) => (
                    <option
                      key={bankAccount.id}
